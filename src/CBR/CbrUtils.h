...
inline std::wstring metaDataFilepathRename(std::wstring filename) {
	std::wstring substringToFind = L"CBRsave\\";
	std::wstring textToInsert = L"Metadata\\";
	size_t position = filename.find(L"CBRsave\\");
	...
	filenameMeta = filenameMeta.substr(0, filenameMeta.size() - 3) + L"met";
	return filenameMeta;
}
