...
std::string getFullCharName(std::string charName) {
    if (charName == "so") { return "Sol(so)"; };
    if (charName == "ky") { return "Ky(ky)"; };
    if (charName == "ma") { return "May(ma)"; }
    if (charName == "mi") { return "Millia(mi)"; }
    if (charName == "ax") { return "Axl(ax)"; }
    if (charName == "po") { return "Potemkin(po)"; }
    if (charName == "ch") { return "Chipp(ch)"; }
    if (charName == "ed") { return "Eddie(ed)"; }
    if (charName == "ba") { return "Baiken(ba)"; }
    if (charName == "fa") { return "Faust(fa)"; }
    if (charName == "te") { return "Testament(te)"; }
    if (charName == "ja") { return "Jam(ja)"; }
    if (charName == "an") { return "Anji(an)"; }
    if (charName == "jo") { return "Johnny(jo)"; }
    if (charName == "ve") { return "Venom(ve)"; }
    if (charName == "di") { return "Dizzy(di)"; }
    if (charName == "sl") { return "Slayer(sl)"; }
    if (charName == "in") { return "I-No(in)"; }
    if (charName == "za") { return "Zappa(za)"; }
    if (charName == "br") { return "Bridget(br)"; }
    if (charName == "ro") { return "Robo-Ky(ro)"; }
    if (charName == "ab") { return "A.B.A(ab)"; }
    if (charName == "os") { return "Order-Sol(os)"; }
    if (charName == "kl") { return "Kliff(kl)"; }
    if (charName == "ju") { return "Justice(ju)"; }
...
json LoadCbrMetadata(std::string PlayerName, std::string PlayerCharacter, std::string OpponentCharacter, std::string ReplayCount,boost::promise<json>& j) {
    ...
    json container;
    for (const auto& dirEntry : recursive_directory_iterator(L"CBRsave")) {
        auto& t = dirEntry;
        std::wstring s = dirEntry.path().wstring();
        auto found = std::string::npos != s.find(L".cbr");
        if (found) {
...
void createCbrMetadata(FileMetadata meta, std::wstring filename) {
...
json readCbrMetadata(std::wstring filename) {
    boost::filesystem::path dir(L"CBRsave\\Metadata");
    if (!(boost::filesystem::exists(dir))) {
        boost::filesystem::create_directory(dir);
    }
