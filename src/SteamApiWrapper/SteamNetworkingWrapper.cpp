...
{ 
	...
	// Log incoming data:
	Packet* packet = (Packet*)pubDest;
	uint64_t senderID = psteamIDRemote->ConvertToUint64();
	//LOG(2, "Recieved packet: Part: %d, Type: %02X, allocated size: %d, packet size: %d, from: %16X, channel = %d\n", test->part, test->packetType, cubDest, *pcubMsgSize, *psteamIDRemote, nChannel);
	//LOG(2, "\t- %s\n", RawMemoryArrayToString((unsigned char*)pubDest, *pcubMsgSize));

	if (g_interfaces.pNetworkManager->IsIMPacket(packet))
	{
		g_interfaces.pNetworkManager->RecvPacket(packet);
		//Force next instructions to skip to next packet
		ret = false;
		*pcubMsgSize = 0;
	}
