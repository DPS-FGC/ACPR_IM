...
bool utils_WriteFileW(const wchar_t* path, void* inBuffer, unsigned long bufferSize, bool binaryFile, bool append)
{
	std::ofstream file;
...
bool utils_ReadFileW(const wchar_t* path, void* outBuffer, unsigned long bufferSize, bool binaryFile)
{
	std::ifstream file;
...
std::wstring sanitize_filename(const std::wstring& filename)
{
	static const std::unordered_set<wchar_t> illegalChars = {
		L'<', L'>', L':', L'"', L'/', L'\\', L'|', L'?', L'*'
	};
	...
	for (wchar_t ch : filename) {
		if (illegalChars.count(ch) || ch < 32) {
			sanitized += L'_'; // replace illegal characters
		}
		else {
			sanitized += ch;
		}
