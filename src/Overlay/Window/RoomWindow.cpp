...
void RoomWindow::Draw()
{
	...
	else if (isOnCharacterSelectionScreen() || isOnVersusScreen() || isInMatch())
	{
		ImGui::VerticalSpacing(10);
		DrawMatchImPlayers();
	}
...
void RoomWindow::DrawMatchImPlayers()
{
	...
	if (g_interfaces.pRoomManager->IsThisPlayerInMatch())
	{
		...
		{
			uint16_t matchPlayerIndex = imPlayer.matchPlayerIndex;//g_interfaces.pRoomManager->GetPlayerMatchPlayerIndexBySteamID(imPlayer.steamID.ConvertToUint64());
