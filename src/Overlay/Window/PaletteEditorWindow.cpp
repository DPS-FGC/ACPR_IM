...
void PaletteEditorWindow::InvertColors(unsigned char* src, unsigned char* dest)
{
	dest[0] = src[2];
	dest[1] = src[1];
	dest[2] = src[0];
...
void PaletteEditorWindow::ShowPaletteBoxes()
{
	...
	for (int i = 0, col = 1; i < NUMBER_OF_COLOR_BOXES; i++)
	{
		...
		if (m_highlightMode)
		{
			InvertColors((unsigned char*)m_paletteEditorArray + curColorBoxOffset, (unsigned char*)invertedColor);
			ImGui::ColorButtonOn32Bit("##PalColorButton", idx, (unsigned char*) invertedColor, m_colorEditFlags);
			InvertColors((unsigned char*)invertedColor, (unsigned char*)m_paletteEditorArray + curColorBoxOffset);
		...
		{
			InvertColors((unsigned char*)m_paletteEditorArray + curColorBoxOffset, (unsigned char*)invertedColor);
			pressed = ImGui::ColorEdit4On32Bit("##PalColorEdit", idx, (unsigned char*)invertedColor, m_colorEditFlags);
			InvertColors((unsigned char*)invertedColor, (unsigned char*)m_paletteEditorArray + curColorBoxOffset);
		}
...
void PaletteEditorWindow::ShowOnlinePaletteResetButton(Player& playerHandle, uint16_t thisPlayerMatchPlayerIndex, const char* btnText)
{
	...
	if (ImGui::Button(buf))
	{
		...
		g_interfaces.pPaletteManager->SetDisableOnlineCustomPalettes(thisPlayerMatchPlayerIndex, true);
	}
	...
	ImGui::SameLine();
	ShowEnableOnlineCustomPalettes(playerHandle, thisPlayerMatchPlayerIndex);
}

void PaletteEditorWindow::ShowEnableOnlineCustomPalettes(Player& playerHandle, uint16_t playerIndex)
{
	char buf[32];
	sprintf(buf, "Disable##dop%d", playerIndex);

	if (ImGui::Checkbox(buf, g_interfaces.pPaletteManager->GetDisableOnlineCustomPalettesRef(playerIndex)))
	{
		if (g_interfaces.pPaletteManager->GetDisableOnlineCustomPalettes(playerIndex))
		{
			CharPaletteHandle& charPalHandle = playerHandle.GetPalHandle();
			g_interfaces.pPaletteManager->RestoreOrigPal(charPalHandle);
		}
		...
		{
			g_interfaces.pRoomManager->RequestPaletteResend();
		}
	}
	ImGui::HoverTooltip("Disable loading custom palettes for this player online.");
}
...
void PaletteEditorWindow::ShowHoveredPaletteInfoToolTip(const IMPL_info_t& palInfo, CharIndex charIndex, int palIndex)
{
	...
	if (creatorLen || descLen || hasBloom || isOnlinePal)
	{
		ImGui::BeginTooltip();
